...
function resolveHandlerEntry(handlerFile) {
    ...
    const resolved = candidates.find((candidate) => (0, fs_1.existsSync)(candidate));
    if (!resolved) {
        throw new Error(`Cannot resolve handler entry for ${handlerFile}`);
    }
    return resolved;
}
class LearnfyraStack extends cdk.Stack {
    constructor(scope, id, props) {
        ...
        if (enableCustomDomains && (!rootDomainName || !hostedZoneId)) {
            throw new Error('Custom domains enabled but rootDomainName/hostedZoneId not provided in stack props.');
        }
        ...
        if (enableCustomDomains) {
            ...
            cloudFrontCertificate = acm.Certificate.fromCertificateArn(this, 'CloudFrontCertificate', props.cloudFrontCertificateArn);
        }
        // ── Tag all resources ─────────────────────────────────────────────────────
        ...
        const distribution = new cloudfront.Distribution(this, 'CloudFrontDistribution', {
            ...
            domainNames: enableCustomDomains && rootDomainName
                ? isProd
        ...
        const generateFn = new aws_lambda_nodejs_1.NodejsFunction(this, 'GenerateFunction', {
        ...
        const downloadFn = new aws_lambda_nodejs_1.NodejsFunction(this, 'DownloadFunction', {
        ...
        // ── Lambda: Auth handler ───────────────────────────────────────────────────
        const authFn = new aws_lambda_nodejs_1.NodejsFunction(this, 'AuthFunction', {
        ...
        const submitFn = new aws_lambda_nodejs_1.NodejsFunction(this, 'SubmitFunction', {
        ...
        const progressFn = new aws_lambda_nodejs_1.NodejsFunction(this, 'ProgressFunction', {
        ...
        const analyticsFn = new aws_lambda_nodejs_1.NodejsFunction(this, 'AnalyticsFunction', {
        ...
        const classFn = new aws_lambda_nodejs_1.NodejsFunction(this, 'ClassFunction', {
        ...
        const rewardsFn = new aws_lambda_nodejs_1.NodejsFunction(this, 'RewardsFunction', {
        ...
        const studentFn = new aws_lambda_nodejs_1.NodejsFunction(this, 'StudentFunction', {
        ...
        const apiMetricByRoute = (metricName, method, resource, statistic = 'Sum') => new cloudwatch.Metric({
            namespace: 'AWS/ApiGateway',
            metricName,
            statistic,
            period: cdk.Duration.minutes(5),
        ...
        const allFunctionErrors = monitoredFunctions.map(({ id, fn }) => fn.metricErrors({ statistic: 'sum', period: cdk.Duration.minutes(5), label: `${id} Errors` }));
        const allFunctionInvocations = monitoredFunctions.map(({ id, fn }) => fn.metricInvocations({ statistic: 'sum', period: cdk.Duration.minutes(5), label: `${id} Invocations` }));
        const allFunctionDurationP95 = monitoredFunctions.map(({ id, fn }) => fn.metricDuration({ statistic: 'p95', period: cdk.Duration.minutes(5), label: `${id} p95 Duration` }));
        dashboard.addWidgets(new cloudwatch.TextWidget({
            width: 24,
            height: 4,
            markdown: `# Learnfyra ${appEnv.toUpperCase()} Backend Observability\n` +
        ...
        dashboard.addWidgets(new cloudwatch.GraphWidget({
            ...
            width: 12,
            height: 6,
            left: monitoredFunctions.map(({ id, fn }) => fn.metricDuration({
                statistic: 'Sum',
                period: cdk.Duration.minutes(5),
                label: `${id} Total Time`,
            })),
        ...
        dashboard.addWidgets(new cloudwatch.TextWidget({
            width: 24,
            height: 3,
            markdown: '## Cost & Anomaly Visibility (DOP-08)\n' +
        ...
        const lambdaTotalInvocations = new cloudwatch.MathExpression({
            ...
            period: cdk.Duration.hours(1),
            usingMetrics: Object.fromEntries(monitoredFunctions.map(({ fn }, i) => [
                `inv${i}`,
        ...
        if (enableCustomDomains) {
            ...
            const apiCertificate = acm.Certificate.fromCertificateArn(this, 'ApiCertificate', props.apiCertificateArn);
...
exports.LearnfyraStack = LearnfyraStack;
