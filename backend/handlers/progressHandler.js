...
/**
 * Returns all worksheets generated by the authenticated user, with status
 * derived by joining against the attempts table.
 *
 * Status logic:
 *   - Has attempt with percentage != null  → "completed" (score = best percentage)
 *   - Has attempt with percentage == null  → "in-progress" (score = null)
 *   - No matching attempt                  → "new" (score = null)
 *
 * @param {Object} decoded - Verified JWT payload (must have .sub)
 * @param {Object} queryStringParameters - { limit?, offset? }
 * @returns {Promise<{ statusCode, headers, body }>}
 */
async function handleMyWorksheets(decoded, queryStringParameters = {}) {
  const db = getDbAdapter();

  // Fetch worksheets and attempts in parallel
  const [allWorksheets, allAttempts] = await Promise.all([
    db.queryByField('worksheets', 'createdBy', decoded.sub),
    db.queryByField('attempts', 'studentId', decoded.sub),
  ]);

  // Build a map of worksheetId → best attempt (highest percentage)
  const bestAttemptMap = new Map();
  for (const attempt of allAttempts) {
    const wid = attempt.worksheetId;
    if (!wid) continue;
    const existing = bestAttemptMap.get(wid);
    if (!existing) {
      bestAttemptMap.set(wid, attempt);
    } else if (
      attempt.percentage != null &&
      (existing.percentage == null || attempt.percentage > existing.percentage)
    ) {
      bestAttemptMap.set(wid, attempt);
    }
  }

  // Derive status for each worksheet
  const worksheets = allWorksheets.map((ws) => {
    const attempt = bestAttemptMap.get(ws.worksheetId);
    let status = 'new';
    let score = null;
    ...
    return {
      ...
      title: ws.title || `${ws.topic || 'Worksheet'} — Grade ${ws.grade || '?'}`,
      subject: ws.subject || 'Math',
      grade: ws.grade || 1,
      topic: ws.topic || '',
      difficulty: ws.difficulty || 'Medium',
      questionCount: ws.questionCount || ws.questions?.length || 0,
      estimatedTime: ws.estimatedTime || '',
      totalPoints: ws.totalPoints || 10,
      status,
      score,
      createdAt: ws.createdAt || '',
      slug: ws.slug || ws.worksheetId,
    };
  });

  // Sort by createdAt descending (newest first)
  worksheets.sort((a, b) => (b.createdAt || '').localeCompare(a.createdAt || ''));

  // Pagination
  const limitVal = Math.min(Math.max(parseInt(queryStringParameters?.limit, 10) || 100, 1), 200);
  const offsetVal = Math.max(parseInt(queryStringParameters?.offset, 10) || 0, 0);
  const paged = worksheets.slice(offsetVal, offsetVal + limitVal);

  return {
    statusCode: 200,
    headers: corsHeaders,
    body: JSON.stringify({ worksheets: paged, total: worksheets.length }),
  };
}
...
export const handler = async (event, context) => {
  ...
  try {
    ...
    if (path.includes('/worksheets/mine') && method === 'GET') {
      return await handleMyWorksheets(decoded, event.queryStringParameters || {});
    }
    ...
    if (isDebug) {
      body._debug = { stack: err.stack, handler: 'progressHandler', statusCode, timestamp: new Date().toISOString() };
    }
