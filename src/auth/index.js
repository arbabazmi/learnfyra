 ...
 *
 * AUTH_MODE=cognito → cognitoAdapter  (Google OAuth only, email/password blocked)
 * AUTH_MODE=hybrid  → mockAuthAdapter (email/password via DynamoDB + Google OAuth via Cognito)
 ...
 * AUTH_MODE=hybrid  → cognitoAdapter        (Google OAuth via Cognito Hosted UI)
...
export function getOAuthAdapter() {
  if (process.env.AUTH_MODE === 'cognito' || process.env.AUTH_MODE === 'hybrid') {
    return cognitoAdapter;
  }
